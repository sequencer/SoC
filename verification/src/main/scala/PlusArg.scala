...
package org.chipsalliance.utils.verification
...
object PlusArg {
  ...
  def apply(name: String, default: BigInt = 0, docstring: String = "", width: Int = 32): UInt = {
    ...
    Module(new plusarg_reader(name + "=%d", default, docstring, width)).out
  }
  ...
  def timeout(name: String, default: BigInt = 0, docstring: String = "", width: Int = 32)(count: UInt): Unit = {
    PlusArgArtefacts.append(name, Some(default), docstring)
    Module(new PlusArgTimeout(name + "=%d", default, docstring, width)).count := count
  }
}
...
object PlusArgArtefacts {
  ...
  private def serializeHelp_cHeader(tab: String = ""): String = artefacts.map {
    case (arg, info) =>
