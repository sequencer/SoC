...
package object misc {
  ...
  implicit class SeqBoolBitwiseOps(private val x: Seq[Bool]) extends AnyVal {
    ...
    private def padZip(y: Seq[Bool], z: Seq[Bool]): Seq[(Bool, Bool)] =
      y.padTo(z.size, false.B).zip(z.padTo(y.size, false.B))
  }
  ...
  /** Similar to Seq.groupBy except this returns a Seq instead of a Map
    ...
    */
  def groupByIntoSeq[A, K](xs: Seq[A])(f: A => K): collection.immutable.Seq[(K, collection.immutable.Seq[A])] = {
    val map = collection.mutable.LinkedHashMap.empty[K, collection.mutable.ListBuffer[A]]
    for (x <- xs) {
      val key = f(x)
      val l = map.getOrElseUpdate(key, collection.mutable.ListBuffer.empty[A])
      l += x
    }
