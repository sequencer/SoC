...
/** An address space description.
  ...
  * @param permissions  the permission attributes of this space. See [[ResourcePermissions]].
  */
...
/** A serializable description of a device.
  * @param name         the resolved name of this device. See [[DeviceRegName]].
...
class SimpleDevice(val devname: String, devcompat: Seq[String])
  ...
  def describe(resources: ResourceBindings): Description = {
    ...
    val names =
      optDef(
        "reg-names",
        named.map(x => ResourceString(DiplomacyUtils.regName(x._1).get)).toList
      ) // names of the named address space
    val regs =
      optDef("reg", (named ++ bulk).flatMap(_._2.map(_.value)).toList) // address ranges of all spaces (named and bulk)
...
trait BindingScope {
  ...
  protected[dts] var resourceBindingFns: Seq[() => Unit] =
    Nil // callback functions to resolve resource binding during elaboration
  protected[dts] var resourceBindings: Seq[(Resource, Option[Device], ResourceValue)] = Nil
...
object BindingScope {
  protected[dts] var active: Option[BindingScope] = None
  protected[dts] def find(m: Option[LazyModule] = LazyModule.getScope): Option[BindingScope] = m.flatMap {
